...
function initGame() {
	...
	function create() {
		...
		starfield = game.add.tileSprite(0,0,clientWidth,clientHeight, 'stars');
		starfield.fixedToCamera = true;

		var playerCollisionGroup = game.physics.p2.createCollisionGroup();
		var enemyCollisionGroup = game.physics.p2.createCollisionGroup();
		var pBulletCollisionGroup = game.physics.p2.createCollisionGroup();
		var eBulletCollisionGroup = game.physics.p2.createCollisionGroup();
		var powerupCollisionGroup = game.physics.p2.createCollisionGroup();
		...
		keys.mute = game.input.keyboard.addKey(Phaser.Keyboard.M);
		keys.pause = game.input.keyboard.addKey(Phaser.Keyboard.P);
		keys.fullscreen = game.input.keyboard.addKey(Phaser.Keyboard.F);

		keys.mute.onDown.add(toggleMute, this);
		keys.pause.onDown.add(togglePause, this);
		keys.fullscreen.onDown.add(toggleFullscreen, this);

		var enemies = game.add.group();
		enemies.enableBody = true;
		enemies.physicsBodyType = Phaser.Physics.P2JS;

		var pBullets = game.add.group();
		pBullets.enableBody = true;
		pBullets.physicsBodyType = Phaser.Physics.P2JS;

		var eBullets = game.add.group();
		eBullets.enableBody = true;
		eBullets.physicsBodyType = Phaser.Physics.P2JS;
		...
		player = game.add.sprite(25, 25, 'player');
			...
			player.body.collides(enemyCollisionGroup, collideEnemy, this);
			player.body.collides(eBulletCollisionGroup, collideEBullet, this);
			player.body.collides(powerupCollisionGroup, collidePowerup, this);
	}
	...
	function update() {
	    ...
	    {
	        starfield.tilePosition.x -= (player.body.velocity.x * 2) * game.time.physicsElapsed;
	    }

	    if (!game.camera.atLimit.y)
	    {
	        starfield.tilePosition.y -= (player.body.velocity.y * 2) * game.time.physicsElapsed;
	    }
	}
